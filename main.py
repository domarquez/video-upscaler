...
app = FastAPI(title="Video Upscaler Pro – FFmpeg (Fix Filter)")
...
def upscale_with_ffmpeg(input_path: Path, output_path: Path):
    cmd = [
        "ffmpeg", "-y",
        "-i", str(input_path),
        "-vf", "scale=min(1920,iw*min(1,(1080/ih))):min(1080,ih*min(1,(1920/iw))):flags=lanczos,boxblur=2:2,fps=30,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:black",
